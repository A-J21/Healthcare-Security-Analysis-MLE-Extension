...
Trains a logistic regression model for financial fraud detection using the
encrypted ML framework. The trained model can then be loaded into MongoDB
for use by the encrypted ML server.

MODEL TYPE:
Logistic Regression (binary classification: legitimate vs fraudulent)

FEATURES:
10 features (transaction amount, time, location, etc.)
...
from DataGenerators.generate_financial_data import generate_financial_data

def train_financial_model():
    """
    Train a logistic regression model for financial fraud detection.
    ...
    if os.path.exists("financial_features.csv") and os.path.exists("financial_labels.csv"):
        print("   Found existing data files, loading...")
        X = pd.read_csv("financial_features.csv", header=None).to_numpy()
        y = pd.read_csv("financial_labels.csv", header=None).to_numpy().reshape((-1,))
    else:
        print("   Data files not found, generating new data...")
        X, y = generate_financial_data(n_samples=5000, fraud_ratio=0.1)
        pd.DataFrame(X).to_csv("financial_features.csv", header=False, index=False)
        pd.DataFrame(y).to_csv("financial_labels.csv", header=False, index=False)
    ...
    print("2. Update loadModelFromCsv.py to use 'FinancialFraud' as model name")
